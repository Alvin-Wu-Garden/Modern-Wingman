...
- 執行時間：2026-07-30 15:25:30 +08:00
- 執行人：mingj
- commit hash：e0daf6b
...
- Project index：status=Partial，manifest=30456bd8ff6e4d168a4479100f00d056
- Graph schema：nodes=18765，edges=34425
...
| A12 | Edge V4 必要 properties | PASS | 34425/34425 | 100% | weight/confidence/reasonCode/evidenceCount/evidenceRef 聚合。 |
| A13 | 禁止 relationship properties | PASS | forbidden=0 / total=34425 | 100% 無禁止欄位 | 檢查 evidenceJson/sourceId/targetId keys。 |
...
| B6 | connected primary communityId | FAIL | 35.42% (4988/14082) | >=90% | 由 active GraphEntity degree/communityId 聚合。 |
...
| C4 | Normalized throughput | NotMeasured | V4=275,036.65 files/min, 6,183.45 MB/min；V3=0.00, 0.00 | files/min 與 MB/min 均不得較可比 V3 下降>50% | 以完整 run elapsed 與 stageMetrics 最大 processed source 數計算，避免跨 stage 重複加總。 |
| C5 | Peak Working Set | NotMeasured | peak=2826104832 bytes；budget=8589934592 bytes | <=preflight budget；無 OOM/process crash | GET index/run peakWorkingSetBytes。 |
...
- Stage metrics：scan=2267ms/files:13133/bytes:309602025
- no-op run：phase=complete；mode=no-op；elapsedMilliseconds=2865。
- normalized throughput：275,036.65 files/min；6,183.45 MB/min。
- Peak RAM：2826104832 bytes；preflight budget=8589934592 bytes。

## 9. Diagnostics 統計

- Node roles：controller-action=4669；type=3621；table=2257；frontend-page=1449；frontend-module=1291；repository=1004；custom-report=662；controller=516；report-data-source=458；report-plugin=410；csv-format=406；function=382；schedule=370；custom-enum=287；menu-feature=246；view=187；approval-feature=151；data-model=132；business-service=73；batch-report=69；scheduled-task=56；product-type=27；procedure=19；custom-product-type=18；report-data-source-group=5
- Edge reasonCode：scriptdom-read=9387；roslyn-invocation=7853；frontend-url=5202；roslyn-route=4641；es-import=3183；db-metadata=861；scriptdom-write=803；es-import=792；db-metadata=678；fk-constraint=420；menu-link=370；db-metadata=123；scriptdom-exec=55；menu-link-base64=27；roslyn-task-name=22；roslyn-invocation=8
- Node kinds：Code=7047；Data=4046；EntryPoint=6174；Feature=1498
- Relationship types：CALLS=11036；DEPENDS_ON=1336；DISPATCHES_TO=27；HANDLES=5455；MAPS_TO=123；READS=9395；ROUTES_TO=5572；TRIGGERS=678；WRITES=803
- Summary progress message：14 個 AI 摘要失敗；結構模板仍可使用。

## 10. FAIL/Warning/Conditional 處置

- FAIL B6：connected primary communityId；actual=35.42% (4988/14082)。
