...
- 執行時間：2026-07-30 15:48:28 +08:00
- 執行人：mingj
- commit hash：e0daf6b
...
- Project index：status=Partial，manifest=149f39449d7f4e0da7ef94f49229296f
- Graph schema：nodes=18760，edges=33772
...
| A6 | PluginReport dispatch | Manual | DISPATCHES_TO=27 | >=24；stub unresolved | dispatch 數已自動量測；stub unresolved 需人工確認。 |
| A7 | frontend-page | PASS | 1449 | >=600 | active graph role 聚合。 |
| A8 | Named View | NotMeasured | 0 | >=T0.3 baseline 90% | 以 DISPATCHES_TO 且 reasonCode=roslyn-view-result 量測；未提供 T0.3 baseline，禁止假 PASS。 |
...
| A12 | Edge V4 必要 properties | PASS | 33772/33772 | 100% | weight/confidence/reasonCode/evidenceCount/evidenceRef 聚合。 |
| A13 | 禁止 relationship properties | PASS | forbidden=0 / total=33772 | 100% 無禁止欄位 | 檢查 evidenceJson/sourceId/targetId keys。 |
| A14 | RMDAL Code | Manual | 813（degree>0: 807） | >=500 且抽樣有關係 | 數量與 connected 已量測；關係抽樣需人工。 |
...
| B6 | connected primary communityId | PASS | 100.00% (13870/13870) | >=90% | 由 active GraphEntity degree/communityId 聚合。 |
...
| C3 | Full Index wall-clock | NotMeasured | V4=0.05min；scope comparable=false；absolute budget=10.00min | <=1.5x；scope 不可比時 <=核准絕對預算 | GET index/run；可比性同時檢查 file/bytes 差異<=10%及明確環境確認。 |
| C4 | Normalized throughput | NotMeasured | V4=282,835.61 files/min, 6,358.78 MB/min；V3=0.00, 0.00 | files/min 與 MB/min 均不得較可比 V3 下降>50% | 以完整 run elapsed 與 stageMetrics 最大 processed source 數計算，避免跨 stage 重複加總。 |
| C5 | Peak Working Set | NotMeasured | peak=3245961216 bytes；budget=8589934592 bytes | <=preflight budget；無 OOM/process crash | GET index/run peakWorkingSetBytes。 |
| C6 | no-op | PASS | phase=complete, mode=no-op | 無變更不重建 | 只讀取最後進度；腳本不會觸發索引。 |
...
| AI4 | Progress API | PASS | total=63, queued=0, running=0, completed=0, failed=63, percent=100 | 欄位正確且狀態總和一致 | GET summaries/progress。 |
| AI5 | UI 分離顯示 | Manual | 未執行 UI 視覺驗收 | 結構可用與 AI 進度分開 | 需桌面 UI 人工確認。 |
| AI6 | Failure 保留 template | Manual | failed=63 | template 保留、無無限 retry | Progress 可見 failed，但 template/retry 明細未由 API 暴露。 |
...
- Stage metrics：scan=2420ms/files:13133/bytes:309602025
- no-op run：phase=complete；mode=no-op；elapsedMilliseconds=2786。
- normalized throughput：282,835.61 files/min；6,358.78 MB/min。
- Peak RAM：3245961216 bytes；preflight budget=8589934592 bytes。

## 9. Diagnostics 統計

- Node roles：controller-action=4664；type=3621；table=2257；frontend-page=1449；frontend-module=1291；repository=1004；custom-report=662；controller=516；report-data-source=458；report-plugin=410；csv-format=406；function=382；schedule=370；custom-enum=287；menu-feature=246；view=187；approval-feature=151；data-model=132；business-service=73；batch-report=69；scheduled-task=56；product-type=27；procedure=19；custom-product-type=18；report-data-source-group=5
- Edge reasonCode：scriptdom-read=9387；roslyn-invocation=7853；roslyn-route=4641；frontend-url=4549；es-import=3183；db-metadata=861；scriptdom-write=803；es-import=792；db-metadata=678；fk-constraint=420；menu-link=370；db-metadata=123；scriptdom-exec=55；menu-link-base64=27；roslyn-task-name=22；roslyn-invocation=8
- Node kinds：Code=7047；Data=4046；EntryPoint=6169；Feature=1498
- Relationship types：CALLS=11036；DEPENDS_ON=1336；DISPATCHES_TO=27；HANDLES=5455；MAPS_TO=123；READS=9395；ROUTES_TO=4919；TRIGGERS=678；WRITES=803
- Summary progress message：63 個 AI 摘要失敗；結構模板仍可使用。

## 10. FAIL/Warning/Conditional 處置

- 自動量測未產生 FAIL；Manual/NotMeasured 仍不得視為通過。
- Manual/NotMeasured 共 54 項，正式驗收前必須補 fixture、人工判定或唯讀 diagnostics。
