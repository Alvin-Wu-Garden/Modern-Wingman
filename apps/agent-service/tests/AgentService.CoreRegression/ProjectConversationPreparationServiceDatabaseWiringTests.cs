...
public sealed class ProjectConversationPreparationServiceDatabaseWiringTests
{
    ...
    public async Task PrepareAsync_專案已設定資料庫連線時_list_database_objects_應真的查詢而非回報未設定()
    {
        ...
        try
        {
            ...
            var service = new ProjectConversationPreparationService(
                ...
                DispatchProxy.Create<IProjectIndexManifestStore, UnusedManifestStoreProxy>(),
    ...
    /// <summary>Graph unavailable 路徑不應讀取版本 manifest。</summary>
    private class UnusedManifestStoreProxy : DispatchProxy
    {
        protected override object? Invoke(MethodInfo? targetMethod, object?[]? args) =>
            throw new InvalidOperationException(
                $"測試不預期呼叫索引 Manifest Store：{targetMethod?.Name}");
    }
}
