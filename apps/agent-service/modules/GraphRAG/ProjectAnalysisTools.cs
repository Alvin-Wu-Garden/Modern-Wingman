...
public sealed class ProjectAnalysisTools
{
    ...
    private const int MaximumToolCalls = 80;
    private const int MaximumGraphSearchCalls = 16;
    private const int MaximumGraphPathCalls = 12;
    private const int MaximumTextSearchCalls = 16;
    private const int MaximumSymbolSearchCalls = 12;
    private const int MaximumFileReadCalls = 32;
    private const int MaximumGraphNodeListCalls = 8;
    private const int MaximumOutlineCalls = 16;
    private const int MaximumDatabaseSchemaCalls = 16;
    private const int MaximumCachedSourceBytes = 64 * 1024 * 1024;
    // 目前權威圖譜允許使用的節點種類，隨 GraphNodeKind 新增或調整需兩邊一起更新；
    // 不直接引用 FblAuthority 的 enum，避免 Tool description 與權威圖譜實作耗合。
    private static readonly string[] SupportedGraphNodeKinds =
    [
        "Menu", "Endpoint", "WebAction", "ViewPage", "ClientScript", "ReactEntry",
        "CodeClass", "CategoryType", "ConfirmSourceType", "DatabaseObject",
        "CustomReportTemplate", "CustomReportDataSource", "CustomParameterDataSource",
        "ReportField",
    ];
    ...
    /// <summary>
    /// 建立本輪 Agent 可呼叫的七個唯讀 Function Tools。
    ...
    [
        ...
        AIFunctionFactory.Create(
            ...
            "適合「這個系統有哪些資料表/選單/Controller」這類列舉型問題，取代對同一分類用不同關鍵字重複呼叫 search_project_graph 亂猜。" +
            "不確定節點屬於哪種 kind 時請先用 search_project_graph，不要自行猜 kind 名稱。"),
        AIFunctionFactory.Create(
        ...
        CancellationToken cancellationToken)
    {
        ...
        return RunToolWithActivityAsync(
            ...
            message => new GraphSearchToolResult(safeQuery, [], message));
    }
        ...
        CancellationToken cancellationToken)
    {
        ...
        return RunToolWithActivityAsync(
            ...
            message => new GraphPathToolResult(safeNodeId, [], false, message));
    }
        ...
        CancellationToken cancellationToken)
    {
        ...
        return RunToolWithActivityAsync(
            ...
            message => new TextSearchToolResult(safeQuery, [], 0, 0, false, message));
    }
        ...
        CancellationToken cancellationToken)
    {
        ...
        return RunToolWithActivityAsync(
            ...
            message => new FileRangeToolResult(safeFilePath, safeStartLine, [], false, message));
    }
        ...
        CancellationToken cancellationToken)
    {
        ...
        return RunToolWithActivityAsync(
            ...
            message => new CSharpSymbolToolResult(safeSymbol, [], 0, false, message));
    }
        ...
        CancellationToken cancellationToken)
    {
        ...
        return RunToolWithActivityAsync(
            ...
            message => new GraphNodeListToolResult(safeKind, [], message));
    }
        ...
        CancellationToken cancellationToken)
    {
        ...
        return RunToolWithActivityAsync(
            ...
            message => new CSharpFileOutlineToolResult(safeFilePath, [], message));
    }
        ...
        CancellationToken cancellationToken)
    {
        ...
        return RunToolWithActivityAsync(
            ...
            message => new DatabaseObjectListToolResult(safeNameFilter, safeKind, [], false, message));
    }
        ...
        CancellationToken cancellationToken)
    {
        ...
        return RunToolWithActivityAsync(
            ...
            message => new DatabaseTableDescriptionToolResult(safeTableName, [], [], message));
    }
        ...
        CancellationToken cancellationToken)
    {
        ...
        return RunToolWithActivityAsync(
            ...
            message => new DatabaseObjectDefinitionToolResult(safeObjectName, null, message));
    }
    ...
    private async Task<T> RunToolWithActivityAsync<T>(
        ...
        Func<string, T> onBudgetExceeded)
    {
        ...
        lock (_toolCacheGate)
        {
            ...
            else
            {
                ...
                if (_totalToolCalls >= MaximumToolCalls)
                    budgetMessage =
                        $"本輪所有工具的總呼叫次數已達上限（{MaximumToolCalls} 次）；請根據已蒐集的證據整理回答，不要再呼叫任何工具。";
                else if (_toolCounts.GetValueOrDefault(category) >= categoryLimit)
                    budgetMessage =
                        $"{tool} 本輪已達 {categoryLimit} 次上限；請改用其他工具，或根據已取得的證據繼續分析。";
                else
                {
                    _totalToolCalls++;
        ...
        if (budgetMessage is not null)
        {
            if (_activity is not null)
            {
                ...
                await _activity.CompleteAsync(budgetActivityId, budgetMessage);
            }
            return onBudgetExceeded(budgetMessage);
        }
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        var files = GetSearchableFiles(normalizedExtension);

        // 大型 .NET Framework 專案動輒上萬個可搜尋檔案；逐一循序讀檔＋掃描一輪可能要 1~2 分鐘，
        // 是專案解析問答拖慢的主因之一，因此改成有界並行掃描，讀檔與逐行比對都能跨核心平行執行。
        var matches = new ConcurrentBag<TextSearchToolItem>();
        var filesScanned = 0;
        var filesSkipped = 0;

        // 湊滿 maxResults 只用「進入前檢查」提早跳過尚未開始的檔案，不對已在平行執行中的
        // 讀檔工作觸發 Cancel()；避免每次搜尋提早收工都對其他 in-flight 檔案讀取丟出一輪
        // OperationCanceledException（偵錯工具會顯示為「未處理例外」雜訊，即使實際上有被吞掉）。
        await Parallel.ForEachAsync(
            files,
            new ParallelOptions
            {
                CancellationToken = cancellationToken,
                MaxDegreeOfParallelism = TextSearchConcurrency,
            },
            async (file, fileCancellationToken) =>
            {
                ...
                var source = await ReadCachedSourceAsync(file, fileCancellationToken);
                if (source is null)
                {
        ...
        return new TextSearchToolResult(
            ...
            filesScanned,
            filesSkipped,
            orderedMatches.Length >= maxResults);
    }
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        {
            // 模型常用舊的 Graph 索引路徑或自行猜測檔名，此時不算異常，回傳正常結果讓模型改用其他工具確認路徑。
