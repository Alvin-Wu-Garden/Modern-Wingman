...
public sealed class Neo4jGraphStore : IGraphStore, IAsyncDisposable
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        return await session.ExecuteReadAsync(async transaction =>
        {
            var nodeCursor = await transaction.RunAsync(
                ...
                RETURN node, degree
                """,
                new { projectId, kinds = normalizedKinds, limit });
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        if (normalizedMode is not ("all" or "in" or "out"))
            throw new ArgumentException("Graph neighbor mode 只允許 all、in、out。", nameof(mode));
                                  ...
                                  selected.Count < limit; level++)
        {
            ...
            foreach (var id in frontier)
            {
                ...
                {
                    if (normalizedMode == "in" && neighbor.Direction != "incoming" ||
                        normalizedMode == "out" && neighbor.Direction != "outgoing")
                        continue;
