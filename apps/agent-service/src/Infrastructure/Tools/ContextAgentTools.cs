...
public sealed class QueryCodeGraphTool(IAgentPolicyEngine policy,IApprovalCoordinator approvals,ICodeGraphStore graph):PolicyEnforcedAgentTool(policy,approvals)
{
    ...
    protected override async Task<ToolExecutionResult> ExecuteCoreAsync(ToolExecutionRequest request,CancellationToken ct){var project=request.Context.ProjectId??throw new ArgumentException("Project context is required.");var query=RequireString(request.Arguments,"query");var hits=await graph.SearchAsync(project,query,20,ct);object output=hits;if(request.Arguments.TryGetValue("nodeKey",out var key)&&key is string nodeKey&&!string.IsNullOrWhiteSpace(nodeKey))output=new{hits,neighborhood=await graph.GetNeighborhoodAsync(project,nodeKey,2,ct)};return new(true,UntrustedContent.Wrap("repository-code-graph",JsonSerializer.Serialize(output)));}
}

public sealed class AnalyzeImpactTool(IAgentPolicyEngine policy,IApprovalCoordinator approvals,ImpactAnalysisService impact):PolicyEnforcedAgentTool(policy,approvals)
{
